...
if __name__ == '__main__':
    ...
    config_all = ConfigParser()
    config_all.read('calendar.config', 'utf-8')
    lists = config_all['accounts']['name'].split(',')
    #读取所有列表

    for acc in lists:
        try:
            config = ConfigParser()
            config.read(f'configs/{acc}.config')
            #读取每一个人的配置文件
            ...
            def check(name, pos):
                if config["classes"]["exclude"] != "True":
                    return False
                #如果不删除某些课程，直接退出
            ...
            for i in range(len(classes)):
                ...
                time_start, time_end = times.split('-')
            ...
            print(f"Finish processing {acc}!")
        except:
            print(f"Error for {acc}!")
